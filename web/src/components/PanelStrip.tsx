...
export function PanelStrip({ rows }: { rows: Supplier[] }) {
  ...
  return (
    <div className="relative" data-strip>
      <ul
      ...
      >
        {sorted.map((s, i) => {
          ...
          const show = (e: { currentTarget: HTMLElement }) => {
            const box = (e.currentTarget.closest('[data-strip]') as HTMLElement).getBoundingClientRect()
            const bar = e.currentTarget.getBoundingClientRect()
            setHover({ s, x: bar.left - box.left + bar.width / 2, y: bar.top - box.top })
          }
          return (
            <li key={s.supplier_id} className="h-full min-w-0 flex-1">
              <button
                type="button"
                aria-label={`${s.supplier_name}, score ${s.score.toFixed(0)}, ${inrShort(s.total_impact)} lost`}
